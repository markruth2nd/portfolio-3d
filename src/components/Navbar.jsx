    ...
    <nav className={`${styles.paddingX} w-full flex items-center py-5 fixed top-0 z-20 bg-primary`}>
      ...
      <Link to="/" className="flex items-center gap-2" onClick={() => {
        ...
        <p className="text-white text-[18px] font-bold cursor-pointer flex">
          Mark &nbsp;
          <span className="sm:block hidden"> Baldry</span>
          </p>
      </Link>
      ...
      {navLinks.map((link) => (
        <li key={link.id} className={`${ active === link.title ? "text-white" : "text-secondary"} font-poppins font-medium cursor-pointer text-`} onClick={() => {
          setToggle(!toggle);
